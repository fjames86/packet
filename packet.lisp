...
(defun pack (object type &key footer)
  "Make a packet buffer from the initial object."
  (let ((flen (length footer))
        (tsize (type-size type)))
    ...
    (let ((buffer (make-array (+ tsize flen)
      ...
      ;; append the binary footer if provided
      (when footer
        (loop for i below flen do
             (setf (elt buffer (+ tsize i))
                   (elt footer i))))
